...
> * -Xms<size>: 힙 메모리의 최소 크기를 설정한다.
> * -Xmx<size>: 힙 메모리의 최대 크기를 설정한다.
> * -XX:MaxGCPauseMillis 옵션을 사용하여 GC 일시 중지 시간을 제어할 수 있습니다. 이 값을 높이면 GC 일시 중지 시간이 증가하지만, GC 빈도가 낮아질 수 있습니다. 
> 그러나 이 값을 너무 높게 설정하면 메모리 사용량이 증가하여 응용 프로그램 성능에 영향을 줄 수 있습니다.
> 일반적으로는 응용 프로그램의 요구 사항과 서비스 수준 목표를 고려하여 적절한 값을 선택해야 합니다. 
> 예를 들어, 대화형 응용 프로그램의 경우 사용자가 기다리는 동안 발생하는 일시 중지 시간을 최소화해야 하므로 더 작은 값을 선택할 수 있습니다. 
> 그러나 백엔드 서비스의 경우 일시 중지 시간이 더 길어도 되지만 GC 빈도가 적어야 할 수 있습니다.
> 일반적으로는 50ms에서 200ms 사이의 값을 선택하는 것이 일반적입니다. 하지만 실제로는 응용 프로그램의 요구 사항과 환경에 따라 다를 수 있습니다. 
> 따라서 이 값을 조정할 때는 성능 테스트와 함께 실제 상황을 고려해야 합니다.
...
### 간단한 JIT 튜닝법
> 단순 JIT 튜닝의 대원칙은 정말 간단하다. '컴파일을 원하는 메서드에게 아낌없이 리소스를 베풀라'는 것이다. 이런 목표를 달성하려면 다음 항목을 점검해야 한다.
> 1. 먼저, PrintCompilation 스위치를 켜고 애플리케이션을 실행한다.
> 2. 어느 메서드가 컴파일됐는지 기록된 로그를 수집한다.
> 3. ReservedCodeCacheSize 를 통해 캐시를 늘린다.
> 4. 애플리케이션을 재실행한다.
...
> -XX:+PrintCompilation
> ```
> 이 스위치를 켜면 컴파일 이벤트 로그가 표준 출력 스트림에 생성되므로 성능 엔지니어는 이 로그를 보고 어떤 메서드가 컴파일되고 있는지 파악할 수 있다.
> 
> **코드 캐시**  
> JIT 컴파일드 코드는 `코드 캐시`라는 메모리 영역에 저장된다. VM 시작 시 코드 캐시는 설정된 값으로 최대 크기가 고정되므로 확장이 불가하다. 코드 캐시가 꽉 차면
> 그때부터 더 이상 JIT 컴파일은 안 되며, 컴파일되지 않은 코드는 인터프리터에서만 실행도니다. 결국 최대로 낼 수 있는 성능에 한참 못미치는 상태로 작동하게 된다.
> ```shell
...
### 코드 캐시 튜닝
> **코드 캐시 크기 설정**  
> -XX:ReservedCodeCacheSize=size 옵션을 사용하여 코드 캐시의 크기를 조정할 수 있습니다. 여기서 size 는 메모리 크기를 지정하는 값입니다(예: 256m).  
> 일반적으로 코드 캐시에 할당되는 메모리는 전체 사용 가능한 메모리의 일부입니다. 이 값을 정확하게 결정하는 것은 어려울 수 있지만, 전체 메모리의 일부를 코드 캐시로 할당하는 것이 합리적입니다.  
> 일반적으로는 전체 메모리의 1%에서 2% 사이의 값을 코드 캐시에 할당하는 것이 권장됩니다. 따라서 1기가 바이트의 1%에서 2%를 계산합니다.  
> * 1%의 경우: 1024MB * 0.01 = 10.24MB
> * 2%의 경우: 1024MB * 0.02 = 20.48MB
> 따라서 코드 캐시에 할당할 적절한 메모리 크기는 약 10MB 에서 20MB 사이가 될 것으로 추정됩니다.  
> 
> **코드 캐시 최적화**
> 코드 캐시의 크기를 늘리는 것 외에도, JVM 의 JIT 컴파일러 옵션을 조정하여 코드 캐시의 효율성을 높일 수 있습니다. 
> 예를 들어, `-XX:+TieredCompilation` 옵션을 사용하면 다양한 최적화 수준에서 컴파일을 수행하여 성능을 향상시킬 수 있습니다.  
> 이 옵션을 사용하면 코드 컴파일의 단계가 여러 단계로 나뉘어집니다. 일반적으로 두 가지 주요 컴파일 단계인 C1 (Client Compiler)과 C2 (Server Compiler)로 나뉩니다. 
> C1은 더 빠르게 컴파일되지만 최적화 수준이 낮고, C2는 더 많은 최적화를 수행하지만 컴파일 시간이 더 오래 걸릴 수 있습니다.  
> `-XX:+TieredCompilation`을 활성화하면 HotSpot 은 처음에는 C1을 사용하여 빠르게 컴파일하고, 코드가 더 자주 실행될수록 C2로 전환하여 더 많은 최적화를 수행합니다. 
> 이는 프로그램 시작 시간을 최소화하면서 동시에 런타임 성능을 최적화하는 데 도움이 됩니다.  
> 
> **코드 캐시 모니터링**
> `-XX:+UseCodeCacheFlushing` 옵션을 사용하여 코드 캐시가 가득 찼을 때 일부 컴파일된 코드를 삭제하고 새로운 코드를 컴파일할 수 있도록 할 수 있습니다. 
> 이는 코드 캐시의 공간을 효율적으로 관리하는 데 도움이 됩니다.
...
> 리스트를 주로 랜덤 엑세스하는 경우라면 ArrayList 가 정답이다. LinkedList 의 고유 기능이 꼭 필요한 경우가 아니라면, 특히 랜덤 엑세스가 필요한 알고리즘을
> 구사할 때에는 ArrayList 를 권장한다. ArrayList 는 가급적 미리 크기를 지정해서 중간에 다시 조정하는 일이 없도록 하는 게 좋다.

### Map 최적화
> **HashMap**  
> HashMap 의 initialCapacity 를 설정하는 것도 ArrayList 경우와 같다. 저장할 데이터 양을 대략이나마 미리 알 수 있으면 그렇게 설정하는 것이 성능상 좋다.

### Set 최적화
> 자바의 Set 은 성능에 관해서 고려해야할 사항은 Map 과 비슷하다.
> 실제로 HashSet 은 HashMAp 으로 구현되어 있다.

### 도메인 객체
> * 가장 흔히 할당되는 자료 구조는 스트링, char 배열, byte 배열, 자바 컬렉션 타입의 인스턴스이다.
> * jmap 에서 누수되는 데이터는 비정상적으로 비대한 데이터셋으로 나타난다.
> 
> 메모리 점유량과 인스턴스 개수 모두 보통 코어 JDK 에 있는 자료 구조가 상위권을 형성하는 게 보통이다. 그런데 애플리케이션에 속한 도메인 객체가 jmap 결과치의
> 상위 30위 정도 안에 든다면 꼭 그렇다고 단정 지을 순 없지만 메모리 누수가 발생한 신호라고 볼 수 있다.  
> 도메인 객체는 비즈니스 관심사를 가장 분명하게, 자연스럽게 나타낸 객체라서 메모리 누수에 더 취약하다. 성능을 중요시하는 개발자는 도메인 객체의 도메인을 인식하고
> 그에 알맞은 크기의 작업 세트가 배정되도록 해야 한다.

---

## 동시 성능 기법
### 실행자와 태스크 추상화
> 일반 자바 프로그래머는 저수준의 스레드 문제를 직접 처리하려고 하기 보다는, java.util.concurrent 패키지에서 적절한 수준으로 추상화된 동시 프로그래밍 지원 기능을
> 골라쓰는 편이 좋다. 아무래도 이 패키지에 들어 있는 클래스를 이용해서 스레드를 바쁘게 돌리는 게 스레드 핫 성능이 더 좋을 것이다.
> 
> 일의 단위를 태스크로 바라보면 동시 프로그래밍을 단순화할 수 있다. 태스크를 실행하는 실제 스레드의 수명주기를 개발자가 일일이 신경 쓸 필요가 없기 때문이다.
> 
> **비동기 실행**  
> 자바에서 태스크를 추상화하는 방법은, 값을 반환하는 태스크를 `Callable` 인터페이스로 나타내는 것이다. Callable<V> 은 call() 메서드 하나밖에 없는 제네릭 인터페이스로,
> call() 메서드는 V 형 값을 반환하되 결괏값을 계산할 수 없으면 예외를 던진다.  
> `Runnable` 은 결과를 반환하거나 예외를 던지지 않는다. Runnable 이 붙잡히지 않는 언체크 예외를 던지면 스택을 거슬러 전파되고 기본적으로 해당 실행 스레드의 실행이 중지된다.  
> 스레드가 살아 있는 동안 예외를 처리하는 일은 프로그래밍 세계의 난제 중 하나로, 정확히 관리하지 않으면 자바 프로그램이 엉뚱한 상태에 빠져버릴지도 모른다.  
> 
> `ExecutorService` 는 관리되는 스레드 풀에서 태스크 실행 메커니즘을 규정한 인터페이스이다. ExecutorService 는 submit() 메서드를 통해 Runnable 또는
> Callable 객체를 받는다.  
> 
> `Executors` 는 헬퍼 클래스로, 선택한 로직에 따라서 서비스 및 기반 스레드 풀을 생성하는 new* 팩토리 메서드 시리즈를 제공한다.
> * newFixedThreadPool(int nThreads): 크기가 고정된 스레드 풀을 지닌 ExecutorService 를 생성한다. 풀 안의 스레드는 재사용되면서 여러 태스크를 실행한다.
> 태스크마다 스레드를 여러 번 생성하는 비용을 줄일 수 있다. 스레드가 전부 사용 중일 경우, 새 태스크는 일단 큐에 보관한다.
> * newCachedThreadPool(): 필요한 만큼 스레드를 생성하되 가급적 스레드를 재사용하는 ExecutorService 를 만든다. 생성된 스레드는 60초간 유지되며, 
> 그 이후는 캐시에서 삭제된다. 이 스레드 풀을 이용하면 소규모 비동기 태스크의 성능을 향상시킬 수 있다.
> * newSingleThreadExecutor(): 스레드 하나만 가동되는 ExecutorService 를 생성한다. 새 태스크는 스레드를 이용할 수 있을 때까지 큐에서 대기한다. 
> 동시 실행 태스크의 개수를 제한해야 할 경우 유용하다.
> * newScheduledThreadPool(int corePoolSize): 미래 특정 시점에 태스크를 실행시킬 수 있도록 Callable 과 지연 시간을 전달받는 메서드들이 있다.
> 일단 태스크가 제출되면 비동기로 처리되며, 태스크를 제출한 코드를 스스로를 블로킹할지, 결과를 폴링할지 선택할 수 있다. ExecutorService 의 submit() 메서드를 호출하면
> Future<V> 가 반환되고, 여기서 get() 또는 타임아웃을 명시한 get() 으로 블로킹하거나, 일반적인 방식대로 isDone() 으로 논블로킹 호출을 한다.
>
> 가장 흔히 사용하는 지표가 코어 수 대비 풀 스레드 수이다. 동시 실행 스레드 개수를 프로세서 개수보다 높게 잡으면 경합이 발생하는 문제가 발생한다. OS 가 스레드들을
> 실행 스케줄링해야 하는 부담을 안게 되어 결국 컨텍스트 교환이 더 자주 일어나게 된다.  
> 경합이 어느 한계치에 이르면 동시 처리 모드로 전환하더라도 성능 효과는 반감될 수 있다. 그래서 성능 모델을 올바르게 정립하고 성능 향상(또는 손실) 정도를 측정할
> 수 있는 역랑을 갖추는 일이 시급하다.

### ForkJoinPool
> ForkJoinPool은 자바의 java.util.concurrent 패키지에 있는 고급 스레드 풀 구현입니다. 이는 주로 병렬 프로그래밍과 관련된 작업에 사용됩니다. 
> ForkJoinPool의 주요 목적은 대규모 작업을 작은 작업으로 나누고, 이를 병렬로 처리하여 전체 작업의 처리 시간을 단축하는 것입니다. 
> 이러한 접근 방식은 "분할 정복(divide-and-conquer)" 알고리즘의 일종으로 볼 수 있습니다.  
> ForkJoinPool의 주요 특징은 다음과 같습니다:
> * 작업 분할(Forking): 큰 작업을 더 작은 작업으로 분할합니다. 이를 통해 작업을 병렬로 처리할 수 있어 성능이 향상됩니다.
> * 결과 합병(Joining): 분할된 작업의 결과를 하나로 합칩니다. 이 과정에서 각각의 작은 작업들이 완료된 후에 최종 결과를 도출합니다.
> * 작업 도둑질(Work-Stealing): ForkJoinPool 내의 스레드는 다른 스레드가 처리해야 할 작업이 없을 때, 다른 스레드의 대기 중인 작업을 '훔쳐' 처리할 수 있습니다. 
> 이를 통해 CPU 사용률을 최적화하고, 스레드 간의 작업 부하를 균등하게 분배합니다.
> * 효율적인 스레드 관리: ForkJoinPool은 필요에 따라 스레드를 생성하고 관리합니다. 스레드 풀의 크기는 작업의 양과 시스템의 자원에 따라 동적으로 조절됩니다.
> 
> ForkJoinPool은 주로 CPU 집약적인 작업에 적합하며, I/O 집약적인 작업에는 적합하지 않을 수 있습니다. 예를 들어, 대규모 배열의 정렬, 복잡한 계산 작업 등에 효과적입니다.
> 이 구현은 자바 7 이상에서 사용 가능하며, java.util.concurrent.ForkJoinTask와 java.util.concurrent.RecursiveTask와 같은 클래스와 함께 사용됩니다. 
> 이들 클래스는 ForkJoinPool 내에서 실행될 수 있는 작업을 정의합니다.

### 병렬 스트림
> Java 8 부터 모든 컬렉션은 stream() 메서드를 제공한다.  
> 두 번째 메서드 parallelStream() 을 이용하면 병렬로 데이터를 작업 후 그 결과를 재조합할 수 있다. 이 메서드를 호출하면 내부적으로 Spliterator 를 써서
> 작업을 분할하고 공용 포크/조인 풀에서 연산을 수행한다.
> 
> 컬렉션이 작을수록 직렬 연산이 병렬 연사보다 훨씬 빠르다. parallelStream() 을 쓸 때에는 꼭 이런 점에 유의하고 반드시 성능 테스트를 해봐야한다.  

---

## 프로파일링
### 개요
> 우리는 넓은 의미에서 (주로 시스템과 그 현재 상태를 살피는 게) 모니터링 툴과 (비정상적/변칙적인 움직임을 감지하는) 경고 시스템, 그리고 
> (실행 중인 애플리케이션의 심층 정보를 제공하는) 프로파일러를 명확히 구분해야 한다. 이 세 가지 툴은 서로 연관되어 있지만 목적 자체가 다르며, 잘 운영되는
> 애플리케이션은 이들 툴을 모두 잘 활용하는 편이다.
> 
> 프로파일링은 대개 애플리케이션을 실행하는 JVM 에 커스텀 에이전트를 부착하여 수행한다.
> 
> 성능 엔지니어는 프로파일링을 수행하기 전에 성능 문제를 밝혀내야 한다. 뿐만 아니라, 성능 문제의 원인이 애플리케이션 코드라는 사실 역시 입증해야 한다.
> 애플리케이션이 유저 모드에서 100%에 육박하는 CPU 사용률을 보이는 경우가 그렇다.
> 
> 이러한 기준이 맞지 않으면 다른 곳에서 문제의 근원을 찾아야 하며, 더 이상 실행 프로파일러로 진단하려고 시도해선 안 된다.
> CPU 가 (커널 시간이 아닌) 유저 모드에서 최고조에 달해 완전 탈진한 상태라면, 프로파일링을 하기 전에 GC STW 같은 요소는 원인 후보군에서 제외시켜야 한다.
> 성능이 중요한 애플리케이션이라면 GC 이벤트를 로깅하고 있을 테니 간단히 확인해볼 수 있다. 머신에 남아있는 GC 로그 및 애플리케이션 로그를 뜯어보고 GC 로그는
> 잠잠한데 애플리케이션 로그는 난리법석인지 알아보면 된다. 난리를 피운 쪽이 GC 로그라면 다음 수순은 실행 프로파일링이 아닌, GC 튜닝이다.

### 개발자용 프로파일링 툴
> **Visual VM**  
> 실행 프로파일러, 메모리 프로파일러가 모두 들어 있고 사용하기 쉬운 무료 툴이다. 기능이 한정된 탓에 운영계 툴로는 거의 안 쓰이지만, 개발/QA 환경에서 애플리케이션
> 작동 상태를 확인하려는 성능 엔지니어에게는 유용한 툴이다.
> 
> 기타 툴
> * JProfiler: ej-테크놀로지스 사가 제작한 유명한 상용 프로파일러.
> * Yourkit: 유어킷 사가 개발한 사용 프로파일러이다.
> * JFR/JMC: 오라클이 BEA 시스템즈 사를 인수하면서 손에 넣은 프로파일링/모니터링 기술이다. 자바 8 기준으로 오라클 JVM 에서만 쓸 수 있는 상용 툴이어서
> OpenJDK 를 비롯한 다른 JVM 에서는 못 쓴다.

### 운영 툴
> 원래 프로파일러는 개발자가 문제점을 진단하거나 애플리케이션이 런타임에 저수준에서 어떻게 작동하는지 파악하려고 사용하지만, 운영계 모니터링 툴로도 많이 쓰인다.
> 현재 시스템 상태를 한눈에 보기 좋게 나타내고 전반적인 시스템 상황이 정상/비정상인지 알려주는 고마운 툴이다.
> 
> 상용 툴로 다음 3가지를 소개한다.  
> **레드햇 서모스탯**  
> 레드햇 사가 개발한 핫스팟 기반의 JVM 전용 오픈 소스 서비서빌리티/모니터링 솔루션이다. OpenJDK 와 사용 라이선스가 같고 단일 머신, 클러스터 모두
> 모니터링할 수 있다. 이력 데이터와 특정 시점별 상태 정보는 몽고DB 에 저장한다.
> 
> **뉴 렐릭**  
> 클라우드 기반 애플리케이션용 SaaS 제품이다. 따라서 비단 JVM 뿐만 아니라 사용 범위가 넓은 범용 툴 세트이다.
> 
> **jClarity 일루미네이트**  
> 개발자 프로파일링 툴과 운영 모니터링 툴 사이의 징검다리 역할을 한다. 실행 중인 JVM 에 뭔가 이상한 점이 발견되면 일루미네이트는 애플리케이션을 철저히 파헤치기 시작한다.  
> 일루미네이트의 머신 러닝 알고리즘은 OS, GC 로그, JVM 에서 긁어모은 데이터를 분석해서 성능 문제를 야기한 근본 원인을 밝힌다.

---

## 자바 9
### 콤팩트 스트링
> Java 9에서 도입된 콤팩트 스트링(Compact Strings) 기능은 자바의 문자열 저장 방식을 개선하여 메모리 사용량을 줄이고 성능을 향상시키는 것을 목적으로 합니다. 
> Java 8 이하에서는 모든 문자열이 char 배열을 사용하여 UTF-16 인코딩으로 저장되었습니다. 
> 이 방식은 각 문자를 표현하기 위해 2바이트(16비트)를 사용했기 때문에, ASCII 문자와 같이 1바이트로 표현 가능한 문자들에 대해 메모리 낭비가 발생했습니다.
> Java 9에서 도입된 콤팩트 스트링은 문자열 내의 모든 문자가 ISO-8859-1 (Latin-1) 인코딩을 사용할 수 있는 경우, 
> 즉 1바이트로 표현 가능한 경우에는 byte 배열을 사용하여 각 문자를 1바이트로 저장합니다. 이는 메모리 사용량을 크게 줄일 수 있습니다. 
> 만약 문자열에 1바이트로 표현할 수 없는 문자가 포함되어 있다면, Java는 UTF-16 인코딩을 사용하여 문자열을 저장합니다.
> 이러한 변화는 대부분의 애플리케이션에 투명하게 적용되며, 개발자가 이 변경사항을 직접 다룰 필요는 없습니다. 
> 콤팩트 스트링 기능은 String 클래스 내부적으로 처리되며, Java 표준 라이브러리를 사용하는 방식에는 영향을 주지 않습니다.
> 이 개선 사항은 특히 문자열 데이터를 많이 사용하는 애플리케이션에서 메모리 사용량을 줄이고, 그 결과로 성능 향상을 기대할 수 있습니다. 
> Java 9 이상을 사용하는 개발자들은 이 새로운 문자열 저장 방식의 혜택을 자동으로 받게 됩니다.
